...
describe("classification", () => {
  const dates = ["01", "03", "05", "07", "09", "11", "13", "15", "17"].map(
    (day) => new Date(`2023-07-${day}T00:00:00.000Z`)
  )
  ...
  describe("Should classify Rover from Ad Libitum correctly", () => {
    ...
    it.each(adLibitumTests)(
      "Classification ($date, $percentile, $calculatedPercentile)",
      ({ score, expected }) => {
  ...
  describe("Should classify Growler/Rover from Hunger correctly", () => {
    const dataPoints = [
      [
        ...
        5,
        8,
        [ClassificationType.Growler, ClassificationType.Rover],
      ],
    ...
    const hungerTests: { score: Score; expected: ClassificationType[] }[] =
      dataPoints.map(
        ([
          date,
          percentile30Minutes,
    ...
    it.each(hungerTests)(
      "Classification ($score.date, $score.calculatedPercentile30Minutes, $score.calculatedPercentile1Hour)",
      ({ score, expected }) => {
        const classifications = classifyUser([score])
        expect(classifications).toMatchSnapshot()
        const classificationTypes = classifications.map(
          ({ classification }) => classification
        )
        expect(sorted(classificationTypes, (x) => x, "ascending")).toEqual(
          expected
        )
      }
    )
  ...
  describe("Should classify Ember from Activity correctly", () => {
    const activityTests: {
      score: Score
      expected: ClassificationType | null
    }[] = (
      [
        [dates[0], 75, 80, null],
        [dates[1], 25, 30, null],
    ...
    it.each(activityTests)(
      "Classification ($score.date, $score.percentile, $score.calculatedPercentile)",
      ({ score, expected }) => {
        const classifications = classifyUser([score])
        expect(classifications).toMatchSnapshot()
        if (expected) {
